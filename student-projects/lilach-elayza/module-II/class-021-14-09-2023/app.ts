...
statusField.addEventListener("change", function () {
  ...
  if (selectedStatus === "discount") {
    ...
    <input type="numeric" id="discount" name="discount" pattern="\d+" minlength="1" maxlength="2" required />
    `;
      ...
      if (e.key === "Enter") {
        ...
        if (!isNaN(index)) {
          ...
          if (newStatus === "discount") {
            const discountSpan =
              status.parentElement!.parentElement!.querySelector(
                ".listedDiscount"
              ) as HTMLElement;
            ...
            discountSpan.addEventListener("keypress", (e: KeyboardEvent) => {
              if (e.key === "Enter") {
                ...
                if (!isNaN(newIndex)) {
                  ...
                  cars[newIndex].discount = newRate;
                }
                discountSpan.blur();
                refreshCarList();
              }
